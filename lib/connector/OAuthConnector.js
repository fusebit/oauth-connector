...
class OAuthConnector {
        ...
        if (data) {
            for (let p in data) {
                ...
                if (match && typeof data[`${match[1]}_oauth_connector_base_url`] === 'string') {
                    userContext.foreignOAuthIdentities = userContext.foreignOAuthIdentities || {};
                    userContext.foreignOAuthIdentities[match[1]] = {
                        userId: data[p],
    ...
    async getUser(fusebitContext, vendorUserId, foreignVendorId) {
        if (foreignVendorId) {
            let data = await fusebitContext.storage.get(this._getStorageIdForVendorUser(vendorUserId, foreignVendorId));
        ...
        if (userContext.foreignOAuthIdentities) {
            for (let foreignVendorId in userContext.foreignOAuthIdentities) {
                await fusebitContext.storage.put(
                    ...
                    this._getStorageIdForVendorUser(userContext.foreignOAuthIdentities[foreignVendorId].userId, foreignVendorId)
                );
            }
        }
    ...
    async deleteUser(fusebitContext, vendorUserId, foreignVendorId) {
        ...
        if (userContext && userContext.foreignOAuthIdentities) {
            for (let fvId in userContext.foreignOAuthIdentities) {
                await fusebitContext.storage.delete(this._getStorageIdForVendorUser(userContext.foreignOAuthIdentities[fvId].userId, fvId));
            }
        }
        return fusebitContext.storage.delete(this._getStorageIdForVendorUser(userContext.vendorUserId));
    }
    ...
    async ensureAccessToken(fusebitContext, userContext, foreignVendorId) {
        ...
        const waitForRefreshedAccessToken = async (count, backoff) => {
            ...
            if (!count > 0) {
                throw new Error(`Error refreshing access token. Waiting for the access token to be refreshed exceeded the maximum time`);
            }
            return new Promise((resolve, reject) => {
                setTimeout(async () => {
                    ...
                    if (userContext.status === 'authenticated') {
                        ...
                        let result;
                        try {
                            result = await this.waitForRefreshedAccessToken(count - 1, Math.floor(backoff * this.refreshBackoffIncrement));
                        } catch (e) {
                            return reject(e);
                        }
